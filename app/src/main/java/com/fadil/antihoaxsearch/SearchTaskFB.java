...
public class SearchTaskFB extends AsyncTask<String, Void, List<GObject>> {

    private AsyncDelegate delegate;
    private long firstItemID = 1;
    private boolean type = false;

    public SearchTaskFB(AsyncDelegate delegate) {
    ...
    protected List<GObject> doInBackground(String... params) {
        ...
        try {
            ...
            list.setKey("AIzaSyBvWOhWflgYkOr6I83wt0XqMdUyHzOARy8");
            list.setCx("016833653733674042761:qx7sfj74sae");
            ...
            if (results.getItems() != null)
                if (type) {
                    ...
                    for (Result result : results.getItems()) {
                        if (result != null)
                            objects.add(new GObject(result.getTitle(), result.getLink()));
                    }
                }
            return objects;
        } catch (IOException e) {
